...
export function CreateBaseResumeDialog({
  ...
  const isSectionSelected = (section: keyof typeof selectedItems): boolean => {
    const sectionItems = profile[section].map((item, index) =>
      getItemId(section, item, index)
    );
    return (
      ...
      const selectedContent = {
        work_experience: profile.work_experience.filter((exp, index) =>
          selectedItems.work_experience.includes(
            getItemId("work_experience", exp, index)
          )
        ),
  ...
  const initializeSelectedItems = () => {
    setSelectedItems({
      work_experience: profile.work_experience.map((exp, index) =>
        getItemId("work_experience", exp, index)
      ),
      education: profile.education.map((edu, index) =>
        getItemId("education", edu, index)
      ),
      skills: profile.skills.map((skill, index) =>
        getItemId("skills", skill, index)
      ),
      projects: profile.projects.map((project, index) =>
        getItemId("projects", project, index)
      ),
    });
  };
            ...
            <div className="space-y-6">
              <div className="text-center space-y-2">
                ...
                <p className="text-gray-600">
                  This helps us tailor your resume content and format
                </p>
              </div>
              ...
              <div className="space-y-4 max-w-md mx-auto">
                ...
                <div className="text-xs text-gray-500 space-y-1">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 ml-4">
                    <li>
                      Be specific (e.g., &ldquo;Frontend Developer&rdquo; vs
                  ...
                  {[
                    {
                      ...
                      icon: Copy,
                      label: "From Profile",
                      desc: "Use existing profile data",
                    },
                    {
                      id: "import-resume",
                      icon: Upload,
                      label: "Import Resume",
                      desc: "Upload or paste resume",
                    },
                    {
                      id: "scratch",
                      icon: Wand2,
                            ...
                            <div className="flex items-center gap-2 px-3 py-2">
                              <Checkbox
                                checked={isSectionSelected(
                                  section.key as keyof typeof selectedItems
                                )}
