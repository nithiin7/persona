...
export default function UsersTable() {
  ...
  const [sortDescriptor, setSortDescriptor] = useState<
    SortDescriptor<SortableColumns>
  >({
    column: "created_at",
  ...
  const uniquePlans = useMemo(() => {
    const plans = new Set(
      users.map((u) => u.subscription?.subscription_plan || "none")
    );
  ...
  const uniqueStatuses = useMemo(() => {
    const statuses = new Set(
      users.map((u) => u.subscription?.subscription_status || "none")
    );
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((item) => {
      ...
      if (searchTerm) {
        ...
        const planText =
          item.subscription?.subscription_plan?.toLowerCase() || "";
        const statusText =
          item.subscription?.subscription_status?.toLowerCase() || "";
    ...
    return [...filteredUsers].sort((a, b) => {
      ...
      switch (column) {
        ...
        case "last_sign_in_at":
          ...
          break;
        case "name":
          valA = `${a.profile?.first_name || ""} ${a.profile?.last_name || ""}`
            .trim()
            .toLowerCase();
          valB = `${b.profile?.first_name || ""} ${b.profile?.last_name || ""}`
            .trim()
            .toLowerCase();
          break;
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>
                    <Button
                      variant="ghost"
                      onClick={() => handleSort("resume_count")}
                      className="px-0 hover:bg-transparent"
                    >
              ...
              <TableBody>
                {sortedUsers.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {formatDate(item.user.last_sign_in_at)}
                    </TableCell>
                    <TableCell>
                      {item.profile?.first_name && item.profile?.last_name
                        ? `${item.profile.first_name} ${item.profile.last_name}`
                    ...
                    <TableCell>
                      {formatDate(item.subscription?.current_period_end)}
                    </TableCell>
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>
                    <Button
                      variant="ghost"
                      onClick={() => handleSort("work_experience_count")}
                      className="px-0 hover:bg-transparent"
                    >
                  ...
                  <TableHead>
                    <Button
                      variant="ghost"
                      onClick={() => handleSort("education_count")}
                      className="px-0 hover:bg-transparent"
                    >
              ...
              <TableBody>
                {sortedUsers.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {item.profile?.work_experience &&
                      Array.isArray(item.profile.work_experience) ? (
                        <Badge variant="outline">
                          {item.profile.work_experience.length} entries
                        </Badge>
                      ) : (
                    ...
                    <TableCell>
                      ...
                      Array.isArray(item.profile.education) ? (
                        <Badge variant="outline">
                          {item.profile.education.length} entries
                        </Badge>
                      ) : (
                        <Badge variant="outline">0</Badge>
                      )}
                    ...
                    <TableCell>
                      ...
                      Array.isArray(item.profile.skills) ? (
                        <Badge variant="outline">
                          {item.profile.skills.length} entries
                        </Badge>
                      ) : (
                        <Badge variant="outline">0</Badge>
                      )}
